...
public sealed class MainForm : Form
{
    ...
    readonly Label detected = new() { AutoSize = true, Text = "Select a game image for automatic detection." };
    ...
    public MainForm()
    {
        ...
        AddRow(grid, 5, "Aspect ratio", aspect, null);
        grid.Controls.Add(new Label { Text = "Available patches", AutoSize = true }, 0, 6); grid.Controls.Add(patches, 1, 6); grid.SetColumnSpan(patches, 2);
        var iconPanel = new FlowLayoutPanel { Dock = DockStyle.Fill, AutoSize = true };
        var iconBtn = new Button { Text = "From file...", AutoSize = true }; iconBtn.Click += (_,__) => ChooseIcon();
        ...
        grid.Controls.Add(new Label { Text = "Icon", AutoSize = true }, 0, 7); grid.Controls.Add(iconPanel, 1, 7);
        ...
        var note = new Label { AutoSize = true, MaximumSize = new Size(650, 0), Text = "The generated disc contains PLAY.exe, the PCSX2 runtime, the BIOS and the game. The target PC does not need PCSX2 or PS2 Builder installed." };
    ...
    { var b = new Button { Text = "Browse..." }; b.Click += async (_,__) => { using var d = new OpenFileDialog { Title = caption, Filter = filter }; if (d.ShowDialog() == DialogResult.OK) { target.Text = d.FileName; if (after != null) await after(); } }; return b; }

    Button BrowseSaveButton()
    { var b = new Button { Text = "Browse..." }; b.Click += (_,__) => { using var d = new SaveFileDialog { Filter = "ISO|*.iso", FileName = string.IsNullOrWhiteSpace(title.Text) ? "PS2Game.iso" : SafeName(title.Text) + ".iso" }; if (d.ShowDialog() == DialogResult.OK) output.Text = d.FileName; }; return b; }

    string? iconPath;
    void ChooseIcon() { using var d = new OpenFileDialog { Filter = "Images|*.ico;*.png;*.jpg;*.jpeg|All files|*.*" }; if (d.ShowDialog() == DialogResult.OK) iconPath = d.FileName; }

    async Task DetectAsync()
    {
        try
        {
            detected.Text = "Analyzing..."; patches.Items.Clear();
        ...
        catch (Exception ex) { detected.Text = "Not recognized: " + ex.Message; }
    }

    async Task BuildAsync()
    {
        if (!File.Exists(game.Text) || !File.Exists(bios.Text) || string.IsNullOrWhiteSpace(output.Text)) { MessageBox.Show("Select a game image, PS2 BIOS and output ISO."); return; }
        ...
        catch (Exception ex) { MessageBox.Show(ex.ToString(), "Build failed", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        finally { progress.Style = ProgressBarStyle.Blocks; build.Enabled = true; }
    }

    async Task ChooseSteamGridIconAsync()
    {
        if (string.IsNullOrWhiteSpace(title.Text)) { MessageBox.Show("Detect a game first."); return; }
        var key = Prompt.Show("SteamGridDB API key", "Enter your SteamGridDB API key (it is not saved):");
        if (string.IsNullOrWhiteSpace(key)) return;
        try
        {
            var urls = await SteamGridDb.FindIconUrlsAsync(title.Text, key);
            if (urls.Count == 0) { MessageBox.Show("No icons found."); return; }
            ...
            MessageBox.Show("SteamGridDB icon selected automatically. You can replace it using 'From file...'.");
        }
