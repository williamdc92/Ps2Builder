...
* Configurable internal rendering resolution:
...
* Writable emulator data is separated from the read-only disc using `-datapath`.
* Shared memory cards stored under:

```text
Saved Games\PS2Builder\MemoryCards\
```

Memory cards are created or initialized by PCSX2 when they do not already exist.

* Per-game configuration and cache stored under:
...
* ISO/UDF image generation through **Windows IMAPI2FS**, without requiring `mkisofs`, ImgBurn or any external disc authoring software.
...
Any selected patches are applied at runtime by PCSX2.
...
* If the target PC does not have the Visual C++ Runtime required by PCSX2, the first launch may trigger a Windows UAC prompt in order to install the bundled official Microsoft redistributable. No download or manual configuration is required.
